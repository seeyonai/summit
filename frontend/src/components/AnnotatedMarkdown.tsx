...
const processAnnotations = (text: string, keyPrefix: string = ''): React.ReactNode[] => {
  ...
  allMatches.forEach((match, matchIndex) => {
    ...
    if (match.type === 'discussion') {
      parts.push(
        <span key={`${keyPrefix}-discussion-${matchIndex}`} className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-100 border border-yellow-300 rounded-md text-yellow-800 text-sm font-medium shadow-sm mx-1 font-sans">
      ...
      parts.push(
        <span key={`${keyPrefix}-todo-${matchIndex}`} className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 border border-blue-300 rounded-md text-blue-800 text-sm font-medium shadow-sm mx-1 font-sans">
...
const AnnotatedMarkdown: React.FC<AnnotatedMarkdownProps> = ({ content, className = '' }) => {
  ...
  const components: Components = {
    ...
    li: ({ children }) => {
      ...
      return <li className="text-gray-700 leading-relaxed">{processedChildren}</li>
    },
    strong: ({ children }) => {
      ...
      if (text.includes('[争论焦点]')) {
        return (
          <span className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-100 border border-yellow-300 rounded-md text-yellow-800 text-sm font-medium shadow-sm font-sans">
            <AlertTriangle className="w-4 h-4 flex-shrink-0" />
      ...
      if (text.includes('[待办事项]')) {
        return (
          <span className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 border border-blue-300 rounded-md text-blue-800 text-sm font-medium shadow-sm font-sans">
            <Circle className="w-4 h-4 flex-shrink-0" />
  ...
  return (
    <div className={`prose prose-sm max-w-none ${className} prose-headings:font-sans prose-p:leading-relaxed prose-p:text-gray-700 prose-li:text-gray-700 prose-strong:text-gray-900 prose-h1:text-gray-900 prose-h2:text-gray-800 prose-h3:text-gray-800`}>
      <ReactMarkdown
