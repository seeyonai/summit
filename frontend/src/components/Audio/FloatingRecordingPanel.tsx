...
function FloatingRecordingPanel({ isVisible, onClose }: FloatingRecordingPanelProps) {
  ...
  // Auto-associate meeting when panel is open and on meeting detail page
  useEffect(() => {
    ...
    if (isMeetingDetailRoute) {
      ...
      if (meetingId && selectedMeeting?._id !== meetingId) {
        apiService
          ...
          .catch((error) => {
            console.error('Failed to fetch meeting for auto-association:', error);
          });
      }
    }
  }, [isVisible, location.pathname, isRecording, selectedMeeting?._id]);
          ...
          {!isRecording ? (
            ...
            <>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div>
                    <MeetingPicker selectedMeeting={selectedMeeting} onMeetingSelect={setSelectedMeeting} disabled={isRecording} />
                  </div>
                </TooltipTrigger>
                <TooltipContent>
          ...
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200">
            ...
            <div className="px-4 py-4">
              ...
              {savedRecordingId && savedFilename && (
                <div className="flex flex-col items-center justify-center py-4">
                  ...
                  >
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate flex-1 text-left">{savedFilename}</span>
            ...
            <div className="px-4 py-3 bg-gray-50 dark:bg-gray-900/50 flex gap-2">
              {!savedRecordingId && !isSaving && (
                <>
                  <Button type="button" variant="outline" onClick={() => setShowStopDialog(false)} className="flex-1 h-9 text-sm">
                    取消
                  </Button>
                  <Button type="button" onClick={handleConfirmStop} className="flex-1 h-9 text-sm">
                    确认停止
                  </Button>
                </>
              )}
              {savedRecordingId && (
                <>
                  <Button type="button" variant="outline" onClick={handleCloseDialog} className="flex-1 h-9 text-sm">
                    关闭
                  </Button>
                  {/* <Button
                  type="button"
