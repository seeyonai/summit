...
interface AssociateNoteWithMeetingDialogProps {
  isOpen: boolean;
  onClose: () => void;
  note: Note;
  onSuccess: () => void;
...
function AssociateNoteWithMeetingDialog({
  isOpen,
  onClose,
  note,
  onSuccess,
  onError
}: AssociateNoteWithMeetingDialogProps) {
  ...
  const handleAssociateToMeeting = async (meetingId: string) => {
    ...
    try {
      ...
      await apiService.associateNoteWithMeeting(note._id, meetingId);
      onSuccess();
      onClose();
    } catch (err) {
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>选择要将此速记关联到的会议</DialogDescription>
        </DialogHeader>
...
export default AssociateNoteWithMeetingDialog;
