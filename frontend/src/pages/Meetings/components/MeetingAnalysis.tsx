...
import { BrainIcon, AlertCircleIcon, RefreshCwIcon, Trash2, ChevronDown } from 'lucide-react';
...
function MeetingAnalysis({ meeting, children, isViewerOnly = false }: MeetingAnaylysisProps) {
  const { analysisResult, loading, error, progress, extractAnalysis, abortExtraction, clearAnalysis, clearDisputedIssues, clearTodos } = useAnalysisResult(meeting._id, {
  ...
  if (empty) {
    return (
      <>
        <AnalysisProgressDialog loading={loading} progress={progress} error={error} onAbort={abortExtraction} />
        <Empty>
          <EmptyHeader>
