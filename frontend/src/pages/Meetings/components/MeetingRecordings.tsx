...
const normalizeRecordingOrder = (recordings: Recording[], order: MeetingRecordingOrderItem[] | undefined): RecordingOrderEntry[] => {
  if (!Array.isArray(recordings) || recordings.length === 0) {
    return [];
  }

  const recordingMap = new Map<string, Recording>(recordings.map((recording) => [recording._id, recording]));
...
const reorderOrderEntries = (
  ...
  const unchanged =
    entries.length === reordered.length &&
...
function MeetingRecordings({ meeting, onViewTranscript, onMeetingRefresh }: MeetingRecordingsProps) {
  ...
  const originalRecordings = useMemo(() => recordingsState.filter((recording) => recording.source !== 'concatenated'), [recordingsState]);
  ...
  const persistOrder = useCallback(
    (nextOrder: RecordingOrderEntry[], fallback: RecordingOrderEntry[]) => {
      if (!meeting._id) {
        return;
      }
  ...
  const applyReorder = useCallback(
    (sourceId: string, targetId: string | null, position: DropPosition) => {
      if (!sourceId) {
        return;
      }
  ...
  }, [meeting._id, recordingPendingRemoval, isRemovingRecording, shouldDeleteRecordingFile, orderEntries, persistOrder, clearRemovalState]);
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatisticsCard
            ...
            label="拼接状态"
            value={originalRecordings.length > 1 ? (concatenatedRecordingState ? '已拼接' : '未拼接') : '-'}
            description={originalRecordings.length > 1 ? (concatenatedRecordingState ? '可查看完整转录' : '多个录音可拼接') : '单个录音无需拼接'}
          />
        </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button onClick={handleConcatenateRecordings} disabled={isConcatenating || originalRecordings.length < 2} className="sm:w-auto">
            ...
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              ...
              {(orderedItems.length > 1 || isSaving) && (
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  {orderedItems.length > 1 && <span>拖动左侧图标调整播放顺序</span>}
                  {isSaving && (
            ...
            {orderedItems.length > 0 ? (
              ...
              <Card className="p-12">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">会议进行中可以开始录音</p>
                </div>
              </Card>
            )}
          </div>
      ...
      <Dialog open={removeDialogOpen} onOpenChange={handleDialogOpenChange}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {recordingPendingRemoval
                ? `确定要将「${
                    (recordingPendingRemoval as any).label || recordingPendingRemoval.originalFileName || recordingPendingRemoval._id
                  }」从当前会议中移除吗？`
                : '确定要移除此录音与会议的关联吗？'}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">你可以仅解除关联，或同时删除该录音文件。</p>
            ...
            {shouldDeleteRecordingFile && <p className="text-xs text-destructive">文件删除后不可恢复，请谨慎操作。</p>}
          </div>
