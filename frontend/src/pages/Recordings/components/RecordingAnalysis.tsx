...
function RecordingAnalysis({
  ...
  const speakerNameMap = useMemo(
    () => buildSpeakerNameMap(speakerNames),
    [speakerNames]
  );
  ...
  const runSpeakerSegmentation = async (oracleNumSpeakers?: number) => {
    try {
      setSegmenting(true);
      const hasHint =
        typeof oracleNumSpeakers === "number" &&
        !Number.isNaN(oracleNumSpeakers);
  ...
  const renderSpeakerTimeline = () => {
    ...
    const maxTime = Math.max(
      ...recording.speakerSegments.map((s) => s.endTime)
    );
    const speakerColors = [
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {Array.from(
            new Set(recording.speakerSegments.map((s) => s.speakerIndex))
          ).map((speakerIndex) => (
            <Badge
              key={speakerIndex}
              className={`${
  ...
  const alignmentTokens = useMemo<AlignmentToken[]>(() => {
    if (
      !Array.isArray(recording.alignmentItems) ||
      recording.alignmentItems.length === 0
    ) {
      return [];
    }
    ...
    return recording.alignmentItems.flatMap((item) => {
      const words =
        typeof item?.text === "string"
          ? item.text.split(/\s+/).filter(Boolean)
          : [];
  ...
  const segmentTexts = useMemo(() => {
    if (
      !Array.isArray(recording.speakerSegments) ||
      recording.speakerSegments.length === 0
    ) {
      return {} as Record<number, string>;
    }

    const organizedSpeeches = Array.isArray(recording.organizedSpeeches)
    ...
    recording.speakerSegments.forEach((segment, index) => {
      ...
      if (overlappingSpeech) {
        const text = (
          overlappingSpeech.polishedText ||
          overlappingSpeech.rawText ||
          ""
        ).trim();
        if (text) {
          result[index] = text;
      ...
      if (alignmentTokens.length > 0) {
        ...
        const segEndMs = Math.max(
          segStartMs,
          Math.round(segment.endTime * 1000)
        );
        const snippet = alignmentTokens
          .filter(
            (token) => token.endMs > segStartMs && token.startMs < segEndMs
          )
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex flex-wrap gap-2">
              <p className="text-sm font-medium text-muted-foreground w-full mb-1">
                快速选择：
              </p>
              {[2, 3, 4, 5, 6].map((num) => (
                <Button
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                {Array.from(
                  ...
                  const segments = recording.speakerSegments.filter(
                    (s) => s.speakerIndex === speakerIndex
                  );
                  ...
                  const percentage =
                    (totalTime / (recording.duration || 1)) * 100;
                      ...
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-12 h-12 rounded-xl ${colors.icon} flex items-center justify-center font-bold text-lg`}
                            >
                              {speakerIndex + 1}
                            </div>
                            <div>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-3 p-3 bg-muted rounded-lg border-l-4 ${
                          speakerColors[
                            segment.speakerIndex % speakerColors.length
                          ]
                        }`}
                      >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <StatisticsCard
                  ...
                  label="平均发言时长"
                  value={formatTime(
                    (recording.speakerSegments?.reduce(
                      (acc, s) => acc + (s.endTime - s.startTime),
                      0
                    ) || 0) / (recording.speakerSegments?.length || 1)
                  )}
