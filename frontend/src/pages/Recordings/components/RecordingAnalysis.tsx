...
import { UsersIcon, ClockIcon, MessageSquareIcon, TrendingUpIcon, BarChart3Icon } from 'lucide-react';
...
export interface RecordingAnalysisHandle {
...
const RecordingAnalysis = forwardRef<RecordingAnalysisHandle, RecordingAnalysisProps>(function RecordingAnalysis(
  { recording, onRefresh, setSuccess, setError },
  ref
) {
  ...
  const runSpeakerSegmentation = useCallback(
    async (oracleNumSpeakers?: number) => {
      try {
        setSegmenting(true);
        const hasHint = typeof oracleNumSpeakers === 'number' && !Number.isNaN(oracleNumSpeakers);
        const { message } = await apiService.segmentRecording(recording._id, hasHint ? { oracleNumSpeakers } : {});
        await onRefresh();
    ...
    [recording._id, onRefresh, setSuccess, setError]
  );

  // Expose runAnalysis to parent via ref (uses auto-detect for pipeline)
  useImperativeHandle(
    ref,
    () => ({
      runAnalysis: () => runSpeakerSegmentation(undefined),
    }),
    [runSpeakerSegmentation]
  );
      ...
      <div className="space-y-4">
          ...
          {recording.speakerSegments.map((segment, index) => {
            ...
            return (
              <div
                ...
                title={`${getSpeakerDisplayName(segment.speakerIndex, speakerNameMap)}: ${formatTime(segment.startTime)} - ${formatTime(
                  segment.endTime
                )}`}
              />
            );
        ...
        <div className="flex flex-wrap gap-2">
          {Array.from(new Set(recording.speakerSegments.map((s) => s.speakerIndex))).map((speakerIndex) => (
            <Badge key={speakerIndex} className={`${speakerColors[speakerIndex % speakerColors.length]} text-white`}>
  ...
  const segmentTexts = useMemo(() => {
    ...
    recording.speakerSegments.forEach((segment, index) => {
      const overlappingSpeech = organizedSpeeches.find(
        (speech) => speech.speakerIndex === segment.speakerIndex && speech.endTime > segment.startTime && speech.startTime < segment.endTime
      );
  ...
  const getSegmentPreview = (index: number) => {
    ...
    return text.length > MAX_SEGMENT_SNIPPET_LENGTH ? `${text.slice(0, MAX_SEGMENT_SNIPPET_LENGTH)}…` : text;
  };
  ...
  const emptyTranscript = !recording.transcription;
  const primaryButton = (
    <Button
      onClick={handleAnalysisClick}
      disabled={segmenting || isTooShort || emptyTranscript}
      size={empty ? 'lg' : 'sm'}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>请输入预期的说话人数量（可选）。如不指定，系统将自动检测。</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              />
              <p className="text-sm text-muted-foreground">常用选项：2人、3人、4人对话等</p>
            </div>
            <div className="flex flex-wrap gap-2">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmAnalysis}>开始分析</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
              ...
              <div className="flex flex-wrap gap-4">
                {Array.from(new Set(recording.speakerSegments?.map((s) => s.speakerIndex))).map((speakerIndex) => {
                  ...
                  const speakerColors = [
                    ...
                    {
                      gradient: 'from-chart-4/20 to-chart-4/20',
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {recording.speakerSegments?.map((segment, index) => {
                    const speakerColors = ['border-primary', 'border-success', 'border-warning', 'border-chart-4', 'border-chart-5'];
                    const preview = getSegmentPreview(index);
                    const fullText = segmentTexts[index];
                    return (
                      <div
                        key={index}
                        className={`flex items-center gap-3 p-3 bg-muted rounded-lg border-l-4 ${
                          speakerColors[segment.speakerIndex % speakerColors.length]
                        }`}
                      >
                        ...
                        <div className="flex-grow min-w-0">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="text-muted-foreground">({formatTime(segment.endTime - segment.startTime)})</span>
                          </div>
                          <div
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <StatisticsCard
                  ...
                  label="平均发言时长"
                  value={formatTime(
                    (recording.speakerSegments?.reduce((acc, s) => acc + (s.endTime - s.startTime), 0) || 0) /
                      (recording.speakerSegments?.length || 1)
                  )}
